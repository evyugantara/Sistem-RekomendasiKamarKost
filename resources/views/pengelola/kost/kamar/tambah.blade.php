...
@section('title', 'Tambah Kamar - ' . $kost->name)
@section('header-title', 'Tambah Kamar Baru')
@section('breadcrumb-active', 'Tambah Kamar')
...
@section('content')
<form action="{{ route('pengelola.kost.kamar.simpan', $kost->id) }}" method="post" enctype="multipart/form-data">
    @csrf
        ...
        <div class="box box-primary">
            ...
            <div class="box-body">
                <div class="form-group">
                    ...
                    <input type="text" name="name" class="form-control" placeholder="Contoh: Kamar A1, Kamar Deluxe" value="{{ old('name') }}" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" class="form-control" rows="3" placeholder="Contoh: Ukuran 3x4m, ventilasi jendela menghadap ke luar...">{{ old('description') }}</textarea>
                </div>

                <div class="form-group">
                    <label>Foto Kamar (Opsional)</label>
                    <input type="file" name="image" class="form-control" accept="image/*">
                    <small style="color: #777; display: block; margin-top: 3px;">jpeg, png, jpg, webp. Maks. 2MB.</small>
                </div>
            </div>
        </div>
                        ...
                        <div class="check-card">
                            ...
                            @forelse($k->opsiKriteria as $o)
                                <div class="check-item">
                                    <input type="checkbox" name="fasilitas[{{ $k->id }}][]" id="fasilitas_{{ $o->id }}" value="{{ $o->id }}" {{ (is_array(old('fasilitas.' . $k->id)) && in_array($o->id, old('fasilitas.' . $k->id))) ? 'checked' : '' }}>
    ...
    <div style="display: flex; gap: 10px; margin-top: 5px;">
        <button type="submit" class="btn-custom btn-success-custom" style="padding: 10px 25px; font-weight: bold; font-size: 14px;">
            <i class="fa-solid fa-save"></i> Daftarkan Kamar
        </button>
